...
export const initializeDatabase = async () => {
  try {
    ...
    if (!cosmosClient) {
      console.log(
        "⚠️  Cosmos DB credentials not provided - using mock data for development"
      );
      return;
    }
...
export const getUsersContainer = (): Container => {
  if (!cosmosClient || !usersContainer) {
    throw new Error(
      "Cosmos DB not configured or not initialized. Please set COSMOS_DB_ENDPOINT and COSMOS_DB_KEY environment variables."
    );
  }
